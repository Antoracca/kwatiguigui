...
// ---------------------------------------------------------------------------
// Proxy — Next.js 16: fichier src/proxy.ts, export nommé "proxy"
// ---------------------------------------------------------------------------
export async function proxy(request: NextRequest) {
  ...
  // 5a. Protect /onboarding — nécessite d'être authentifié (OAuth user)
  if (pathname === ONBOARDING_ROUTE && !isAuthenticated) {
    const loginUrl = new URL("/login", request.url);
    loginUrl.searchParams.set("callbackUrl", pathname);
    return NextResponse.redirect(loginUrl);
  }

  // 5b. Protect dashboard routes — redirect to login if not authenticated
